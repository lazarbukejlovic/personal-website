...
async function sendEmail(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Email API request failed (${response.status}): ${message}`)
  }
}
...
Deno.serve(async (req) => {
  ...
  const sendUrl = Deno.env.get('EMAIL_SEND_URL') ?? 'https://personal-website.vercel.app/api/email/send'
